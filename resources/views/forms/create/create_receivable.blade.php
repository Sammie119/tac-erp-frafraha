<form method="POST" action="{{ route('receivable_store') }}">
    @csrf
        ...
        <thead>
            <tr>
                <td colspan="2">
                    ...
                    <x-input-datalist :options="$suppliers" :placeholder="'Enter Supplier'" name="supplier_name"
                                      :list="'suppliers'" autofocus/>
                </td>
                <th colspan="10">
        ...
        $(addButton).click(function(){
            ...
            if(x < maxField){
                $(wrapper).append(`<tr class="align-middle">
                    <td>${x}</td>
                    <td>
                        <input type="text" id="receivable_${x}" list="products" class="form-control receivable_${x}" required>
                        <input type="hidden" name="receivable[${x}][product_id]" id="receivable_${x}-hidden">
                    </td>
                    <td>
                        <input type="text" class="form-control" name="receivable[${x}][description]" required>
                    </td>
                    <td>
                         <input type="number" class="form-control" name="receivable[${x}][quantity]" required>
                    </td>
                    <td>
                        <input type="number" class="form-control" name="receivable[${x}][unit_price]" required>
                    </td>
                    <td>
                        <input type="text" class="form-control" name="receivable[${x}][amount]" required>
                    </td>
                ...
                const el = document.querySelector(`.receivable_${x}`);
                if (el) {
